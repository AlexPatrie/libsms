...
async def test_run_simulation(max_retries: int, delay: float) -> None:
    config_id = "sms"
    experiment = await api._run_simulation(config_id=config_id, max_retries=max_retries, delay_s=delay, verbose=True)
...
async def test_get_status(example_experiment: dm.EcoliExperiment) -> None:
    ...
    status = await api._check_simulation_status(experiment=experiment)
...
async def test_get_analysis_manifest(example_experiment: dm.EcoliExperiment) -> None:
    experiment = example_experiment
    manifest = await api._get_analysis_manifest(experiment=experiment)
